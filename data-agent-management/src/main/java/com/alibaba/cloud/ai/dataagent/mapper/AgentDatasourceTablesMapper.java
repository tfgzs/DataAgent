...
public interface AgentDatasourceTablesMapper {
	...
	// 选择当前智能体数据源所有表
	@Select("select table_name from saad_agent_datasource_tables where agent_datasource_id = #{agentDatasourceId}")
	List<String> getAgentDatasourceTables(@Param("agentDatasourceId") int agentDatasourceId);

	// 删除当前列表中不存在的表
	@Delete("<script>" + "DELETE FROM saad_agent_datasource_tables WHERE agent_datasource_id = #{agentDatasourceId}"
	...
	// 删除当前智能体数据源所有表
	@Delete("DELETE FROM saad_agent_datasource_tables WHERE agent_datasource_id = #{agentDatasourceId}")
	int removeAllTables(@Param("agentDatasourceId") int agentDatasourceId);

	// 插入用户选择的列表
	@Insert("<script>" + "INSERT IGNORE INTO saad_agent_datasource_tables (agent_datasource_id, table_name) VALUES "
